...
意图与格式：
1. 记备忘 - 「备忘 xxx」「记一下 xxx」「别忘了 xxx」。若消息包含多条事项（换行、编号、分号分隔），action: add_memos，params: items（字符串数组，每条一个元素）。单条时 action: add_memo，params: content（备忘内容）, 可选 reminder_date（YYYY-MM-DD）, 可选 thread（工作线程标签）。
...
- 多条备忘：{"action":"add_memos","params":{"items":["写周报 #工作","订机票 #出差","给设计师发邮件"]},"reply":""}
