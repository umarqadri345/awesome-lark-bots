...
{context[:30000] if len(context) > 30000 else context}"""
    ...
    if decomposition:
        # Phase 1 已完成搜索，Phase 2 只需格式化——直接调用，不走 AgentLoop
        try:
            raw = chat_completion(provider="deepseek", system=refine_sys, user=user, temperature=0.7)
        except Exception as e:
            print(f"  [Seed生成] 直接调用失败({e})", flush=True)
            raw = ""
    else:
        # Phase 1 跳过，Phase 2 需要自行搜索——走 AgentLoop
        try:
            tools = [WEB_SEARCH_TOOL, NEWS_SEARCH_TOOL, TRENDING_TOOL, SEARCH_PLATFORM_TOOL] + _collect_skill_tools()
            agent = AgentLoop(
                provider="deepseek",
                system=refine_sys,
                max_rounds=5,
...
def run_brainstorm(
    ...
    if is_v3 and round_summaries:
        ...
        # 创意全清单在后台线程生成，同时主线程开始最终交付
        _inventory_holder: list[str] = [""]
        def _gen_inventory():
            try:
                print("[创意全清单] 提取所有创意方向并排序...", flush=True)
                _inventory_holder[0] = chat_completion(
                    provider="deepseek", system=_inventory_system, user=summary_input, temperature=0.3,
                ).strip()
                print("[创意全清单] 完成", flush=True)
            except Exception as _inv_err:
                print(f"  [创意全清单] 生成失败: {_inv_err}", flush=True)
        _inv_thread = threading.Thread(target=_gen_inventory, daemon=True)
        _inv_thread.start()

        _inv_thread.join(timeout=60)
        _inventory_for_handoff = _inventory_holder[0]
        if _inventory_for_handoff:
            summary_input += f"\n\n━━ 创意全清单（所有方向，含被淘汰的）━━\n{_inventory_for_handoff}"
            print("[创意全清单] 已注入到最终交付上下文", flush=True)
        else:
            print("[创意全清单] 未生成，最终交付将不含完整创意清单", flush=True)
        ...
        if topic_type == "campaign":
            ...
            final_system = (
                ...
                "- 附上完整的创意清单（包含所有被淘汰的方向）——上方「创意全清单」中有每个方向的核心思路、淘汰原因和值得保留的亮点，将这些信息完整写进 prompt\n"
                "- 概述讨论中的关键分歧和洞察（不同角色在哪些点上产生了有价值的碰撞）\n"
                "- 在 prompt 末尾加入：'以上是团队脑暴的完整产出。你拥有我们没有的知识和分析能力——请独立评估所有方向（包括被我们淘汰的），结合你的判断重新分析，然后给出完整的执行计划与工作流。不必受我们的淘汰决定约束。'\n"
            ...
            final_system = (
                ...
                "- 附上完整的创意清单（包含所有被淘汰的方向）——上方「创意全清单」中有每个方向的核心思路、淘汰原因和值得保留的亮点，将这些信息完整写进 prompt\n"
                "- 概述讨论中的关键分歧和洞察（不同角色在哪些点上产生了有价值的碰撞）\n"
                "- 在 prompt 末尾加入：'以上是团队脑暴的完整产出。你拥有我们没有的知识和分析能力——请独立评估所有方向（包括被我们淘汰的），结合你的判断重新分析，然后给出完整的 MVP 技术方案与实现路径。不必受我们的淘汰决定约束。'\n"
            ...
            final_system = (
                ...
                "- 附上完整的观点清单（包含所有被淘汰的立场）——上方「创意全清单」中有每个立场的核心论点、淘汰原因和值得保留的亮点，将这些信息完整写进 prompt\n"
                "- 说明讨论中的关键分歧点和已达成的共识\n"
                "- 在 prompt 末尾加入：'以上是团队策略讨论的完整产出。你拥有我们没有的知识和分析能力——请独立评估所有观点（包括被我们淘汰的），结合你的判断重新分析，然后给出完整的策略框架与验证路径。不必受我们的淘汰决定约束。'\n"
            ...
            final_system = (
                ...
                "- 附上完整的创意清单（包含所有被淘汰的方向）——上方「创意全清单」中有每个方向的核心思路、淘汰原因和值得保留的亮点，将这些信息完整写进 prompt\n"
                "- 概述讨论中的关键分歧和洞察（不同角色在哪些点上产生了有价值的碰撞）\n"
                "- 在 prompt 末尾加入：'以上是团队讨论的完整产出。你拥有我们没有的知识和分析能力——请独立评估所有方向（包括被我们淘汰的），结合你的判断重新分析，然后给出完整的行动方案与风险对策。不必受我们的淘汰决定约束。'\n"
    ...
    # ── 等待创意全清单线程完成并推送 ──
    if is_v3 and round_summaries:
        _inv_thread.join(timeout=45)
        _inventory = _inventory_holder[0]
        if _inventory:
