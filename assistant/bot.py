...
_SMART_CHAT_SYSTEM = (
    "你是「助理bot」，飞书上的工作助手。\n\n"
    "██ 核心规则 ██\n"
    "当用户问到和「工作、任务、优先级、进展、手头的事、接下来做什么」相关的任何问题时，"
    "你必须先调用 get_user_context(scope='all') 获取用户的真实待办、项目和团队动态，"
    "然后基于实际数据回答。绝对不要在没有查数据的情况下给通用建议或反问用户列清单。\n\n"
    "当用户问需要实时信息的问题（新闻、市场、行业）时，先用 web_search 或 news_search 搜索。\n\n"
    "只有纯闲聊或常识问题才直接回答，不调工具。\n\n"
    "██ 回复风格 ██\n"
    "- 直接给结论和具体建议，不要输出框架、理论、方法论\n"
    "- 不要先说「好的」「我来帮你」\n"
...
def _welcome(open_id: str = "") -> dict:
    ...
    sections.append({"text": "直接说你想做什么就行，下面是常用功能："})
    sections.append({"divider": True})
    sections.append({"text": (
        "**📝 任务管理**\n"
        "· `备忘 内容 #标签` — 记待办，多条用换行或分号\n"
        "· `@claude` 标记 — 如 `备忘 调研竞品 @claude`，Claude 自动抓取执行\n"
        "· `完成 1` / `删掉 交报告` — 管理待办\n"
        "· `线程` 看工作线 · `看板` 导出飞书表格（每天自动同步两次）"
    )})
    sections.append({"divider": True})
    sections.append({"text": (
        "**📁 项目 & 财务**\n"
        "· `创建项目 名称` → 多维表格统一管理\n"
        "· `记账 午餐 35 #Q2营销` · `本月花费` · `项目 总览`"
    )})
    sections.append({"divider": True})
    sections.append({"text": (
        "**🔍 研究 · 🌐 翻译**\n"
        "· `研究 xxx` 深度调研 · `fact check xxx` 事实核查\n"
        "· `翻译 内容` 自动判断方向 · `帮我写英文 xxx`"
    )})
    sections.append({"divider": True})
    sections.append({"text": (
        "**📅 日程 & 报告 · 👥 团队**\n"
        "· `明天下午3点开会` 自动建日历 · 每天自动晨报/收尾\n"
        "· `创建团队 名称` · `加入团队 ABC123` · `我的团队`"
    )})
    sections.append({"divider": True})
    sections.append({"note": "发「帮助」看完整指令 · 其他话当聊天回复"})
...
def _help() -> dict:
    return help_card("助理bot", [
        ("📝 备忘 & 看板",
         "**记备忘**\n"
         "· `备忘 内容 #标签` — 标签可选，多条用换行或分号分隔\n"
         "· `完成 1` / `完成 1,2,3` 批量完成 / `做完了 关键词` 模糊匹配\n"
         "· `删除 1` / `删掉 关键词` — 按序号或内容\n"
         "· `备忘列表` 看全部 · `线程` 看工作线分组\n\n"
         "**看板（飞书表格）**\n"
         "· `看板` 导出全部 · `看板 #标签` 导出指定线程\n"
         "· 新备忘实时追加，每天 08:30 / 18:30 自动全量同步\n"
         "· 分区：今日新增 / 本周进行中 / 等待跟进 / 已完成"),
        ("🤖 @claude 任务委派",
         "· 备忘中加 `@claude` 自动标记为 Claude 任务\n"
         "· 示例：`备忘 调研竞品定价 @claude #dev`\n"
         "· Claude 定时抓取 → 分析优先级 → 自动执行\n"
         "· 结果回写看板「Claude备注」字段\n"
         "· 适合：代码调研、文档整理、数据分析、竞品分析\n"
         "· 需人工处理的任务会标注优先级建议"),
        ("🔍 联网研究",
         "· `研究 xxx` — 多来源搜索、交叉验证、结构化报告\n"
         "· `fact check xxx` — 针对性事实核查"),
        ("🌐 翻译 & 英文写作",
         "· `翻译 内容` — 自动判断中↔英方向\n"
         "· `翻成英文/中文 内容` — 指定方向\n"
         "· `翻译（邮件）内容` / `翻译（PPT）内容` — 场景适配\n"
         "· `帮我写英文 xxx` / `帮我用英文回 xxx`\n"
         "· 支持长文多段落，直接贴入"),
        ("📁 项目 & 财务",
         "**项目**\n"
         "· `创建项目 名称` → 多维表格管理，引导设预算\n"
         "· `名称 加任务 内容` · 粘贴会议纪要 → AI 提取任务\n"
         "· `项目列表` · `名称 总览` 查看\n\n"
         "**财务**\n"
         "· `记账 午餐 35 #Q2营销` — 粘贴清单可批量识别\n"
         "· `创建预算 名称` · `本月花费` · `名称 预算` 执行对比\n"
         "· `名称 设目标 xxx 数量 单位` 添加 KPI"),
        ("📅 日程 & 报告",
         "· `明天下午3点开会` — 自动创建飞书日历\n"
         "· `今天` / `明天` — 查看日程\n"
         "· 每天 08:00 自动晨报 · 18:00 收尾提醒\n"
         "· `周报` 本周汇总 · `月报` 全维度总结"),
        ("👥 团队协作",
         "· `创建团队 名称` — 生成 6 位团队码\n"
         "· `加入团队 ABC123` · `我的团队` · `团队码`\n"
         "· `切换到 团队名` · `离开团队`"),
