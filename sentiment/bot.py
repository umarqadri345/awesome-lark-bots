...
def _welcome() -> dict:
    return welcome_card(
        "舆情监控机器人",
        "从 **微博、抖音、小红书、B站** 等 15 个社媒平台 + 🔍 **全网搜索** 采集数据，"
        "单次可达数千条，生成结构化文件供 AI 深度分析。",
        examples=[
            "周报 — 一键生成舆情周报",
            "采集 我的品牌 @微博 @B站 7天 — 自定义采集",
            "平台 — 查看所有可用平台",
        ],
        hints=["加「+分析」可附 AI 分析报告", "联网搜索自动补量 ~1000 条", "发送「帮助」查看完整说明"],
    )
...
def _help() -> dict:
    return help_card("舆情机器人", [
        ...
        ("自定义采集",
         "格式：`采集 关键词 @平台 天数 条数`\n\n"
         "> 采集 新品发布 用户反馈 @微博 @B站 7天\n"
         "> 采集 行业趋势 @全平台 3天 200条\n"
         "> 采集 品牌口碑 @抖音 @小红书 14天 50条\n\n"
         "关键词空格分隔 · 不填平台默认国内6平台 · @全平台=15个"),
        ("🔍 联网搜索补量",
         "采集时自动通过 Tavily + DuckDuckGo 搜索全网内容\n"
         "每个关键词 × 8 种查询变体，目标补量 **~1000 条**\n"
         "搜索结果自动识别来源平台并与社媒数据合并去重\n"
         "环境变量 `WEB_SEARCH_ENABLED=0` 可关闭"),
        ("可选项",
